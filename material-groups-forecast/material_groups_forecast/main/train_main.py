...
from material_groups_forecast.conf.config_getter_impl import ConfigGetterImpl
...
class TrainMain(TrainArguments):
    ...
    def run(self):
        env_vars_getter = ConfigGetterImpl().get(f"{ROOT_PATH}/conf/env_variables.json")
        credentials = env_vars_getter["bigquery_credentials"]
        bigquery_resource = BigQueryResourceImpl(credentials, "tmp")
